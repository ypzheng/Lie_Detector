...
def predict(window):
    ...
    audio_window = window[1:-1]
    X = feature_extractor.extract_features(np.asarray(audio_window))
    ...
    print("Truth or Lie : {}".format(speaker))
...
#    index = 0 # to keep track of how many samples we have buffered so far
    ...
    audio_data_buffer = np.zeros((0,8000))
    heart_data_buffer = np.zeros((0,1))
    while True:
        try:
            ...
            for json_string in json_strings:
                ...
                if (sensor_type == u"SENSOR_AUDIO"):
                    ...
                    audio_data_buffer = np.append(audio_data_buffer, np.array([audio_buffer]), axis=0)
                if (sensor_type == u"SENSOR_HR"):
                    t = data['data']['t']
                    heart_buffer = data['data']['value']
                    #print heart_buffer
                    heart_data_buffer = np.append(heart_data_buffer,np.array([[heart_buffer]]),axis=0)
                
                
                if(audio_data_buffer.shape[0] > 0 and heart_data_buffer.shape[0]>0):
                    window = np.append(audio_data_buffer[0],heart_data_buffer[0], axis=0)
                    audio_data_buffer=np.delete(audio_data_buffer,0,0)
                    heart_data_buffer=np.delete(heart_data_buffer,0,0)
                    print("Received data of length {}".format(len(window)))
